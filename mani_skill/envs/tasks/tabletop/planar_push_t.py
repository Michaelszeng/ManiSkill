...
class PlanarPushTEnv(BaseEnv):
    ...
    **Task Description:**
    ...
    2. Move the end-effector to the end-zone which terminates the episode. [2 Not required for PushT-easy-v2]
    ...
    **Success Conditions:**
    ...
    def __init__(self, *args, robot_uids="panda_stick", robot_init_qpos_noise=0.02, intersection_thresh=None, **kwargs):
        ...
        # This allows us to actively correct orientation in the action
    ...
    def _preprocess_action(self, action):
        """
        Preprocess 2D actions to 6D pose actions with constrained z and orientation.
        ...
        # Create 6D action: [dx, dy, dz, rx, ry, rz]
        full_action = torch.zeros((batch_size, 6), dtype=action.dtype, device=action.device)

        # Set xy position deltas from the 2D action
    ...
    def step(self, action):
        """Override step to preprocess 2D actions to 6D pose commands"""
        # Preprocess 2D action to 6D pose action (with z and orientation correction)
        full_action = self._preprocess_action(action)

        # Call parent step with the augmented 6D action
    ...
    def _load_scene(self, options: dict):
        ...
        def create_tee(name="tee", target=False, base_color=TARGET_RED):
            ...
            # for the second block (vertical part), we translate y by
            # 4*(box1_half_h)-com_y to align flush with horizontal block
    ...
    def pseudo_render_intersection(self):
        ...
        # the indices where the egocentric T is is still the indices of the T in the uv map
        # (indices of uv map never changed, just values)
        ...
        # # ensure no out of bounds indexing - it's fine to not fully 'render' tee
        # # just need to fully see goal tee which is insured
        # # because we are in the goal tee frame, and 'cad' tee render setup
        # # of egocentric view includes full tee
        # # also, the reward isn't miou, it's intersection area / goal area
        # # don't need union -> don't need full T 'render'
        # # ugly solution for now to keep parallelism no loop
        # # set out of bound image t indices to [0,0]
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # goal tee is alredy at y = -0.1 relative to robot,
            # so we allow the tee to be only -0.2 y relative to robot arm
    ...
    def compute_dense_reward(self, obs: Any, action: Array, info: dict):
        ...
        current_quat = self.agent.tcp.pose.q  # (batch, 4)
        batch_size = current_quat.shape[0]

        # Expand target quaternion to batch size
        # target_quat = torch.tensor([0, 1, 0, 0], dtype=torch.float32)
        # target_quat_batched = target_quat.to(device=current_quat.device).unsqueeze(0).expand(batch_size, -1)
        target_quat_batched = self._target_quat.to(device=current_quat.device).unsqueeze(0).expand(batch_size, -1)

        # Compute orientation alignment using quaternion dot product
        # For unit quaternions, dot product gives cos(angle/2) where angle is the rotation between them
        # Due to double cover, need to take absolute value
        quat_dot = torch.abs(torch.sum(current_quat * target_quat_batched, dim=-1))
        # quat_dot ranges from 0 (opposite) to 1 (aligned), we want it as a [0, 1] reward
        orientation_alignment = quat_dot  # already in [0, 1]

        # Scale by proximity to tee to prevent exploitation
        proximity_weight = 1 - torch.tanh(5 * tcp_to_push_pose_dist)
        reward += (orientation_alignment * proximity_weight) / 50
